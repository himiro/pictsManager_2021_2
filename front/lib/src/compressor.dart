...
class Compressor {
  ...
  static Future<String> compressImage(CompressObject object) async {
    return compute(_compressorCompress, object);
  }

  static Future<dynamic> compressImageQueue(CompressObject object) async {
    final response = ReceivePort();
    await Isolate.spawn(_compressorCompressQueue, response.sendPort);
  ...
  static Future<List<String>> compressImageList(
    ...
    return compute(_compressorCompressList, objects);
  }

  static void _compressorCompressQueue(SendPort port) {
    final rPort = ReceivePort();
    port.send(rPort.sendPort);
    rPort.listen((message) {
      final send = message[0] as SendPort;
      final object = message[1] as CompressObject;
      send.send(_compressorCompress(object));
    });
  }

  static List<String> _compressorCompressList(List<CompressObject> objects) {
    var results = [];
    objects.forEach((_o) {
      results.add(_compressorCompress(_o));
    });
    return results;
  }
  ...
  static String _compressorCompress(CompressObject object) {
    ...
    if (isJpg)
      ...
      throw Exception("compressor don't support this image type");
